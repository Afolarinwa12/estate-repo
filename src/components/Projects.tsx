...
const Projects = () => {
  ...
  const [cardsToShow, setCardsToShow] = useState(1);{/*this updates the project carousel image
    1 slide for small screen 4slides for large screen*/}
    ...
    }; {/*makes the next and previous icon functional */}
...
<div className='flex justify-end items-center mb-8'>
 ...
 <div className='overflow-hidden'>
    ...
    style={{transform: `translateX(-${(currentIndex * 100) / cardsToShow}%)`}}>
      {/*allows the cards to slide 25% */}
    
       {projectsData.map((project, index)=>(
        <div key={index} className='relative flex-shrink-0 w-full sm:w-1/4'>{/*displays 4 images in one role */}
            ...
            <div className='inline-block bg-white w-3/4 px-4 py-2 shadow-md'>
                ...
                <p className='text-gray-500 text-sm'>
                  {project.price} <span className=''></span>{project.location}
                </p>
            </div>
            </div>
        </div>
       ))} {/*"fetches Project data from assets.js to reduce long coding"*/}
